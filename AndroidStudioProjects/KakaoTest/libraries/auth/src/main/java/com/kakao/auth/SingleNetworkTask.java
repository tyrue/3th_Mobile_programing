        ...
        if (getErrorCode(errResponseBody) == ErrorCode.INVALID_TOKEN_CODE) {
            ...
            if (session.getTokenInfo().hasRefreshToken()) {
                ...
                if (futureToken == null) {
                    ...
                    throw new KakaoException(KakaoException.ErrorType.AUTHORIZATION_FAILED, "Could not refresh access token with refresh token. Refresh token might have expired. User has to login again.");
                }
                retry = futureToken.hasValidAccessToken();
            }
